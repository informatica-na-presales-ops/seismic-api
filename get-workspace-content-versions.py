...
log = notch.make_log('seismic_api.get_workspace_content_versions')


def batch_upsert_records(cur, records):
    sql = '''
        insert into seismic_workspace_content_versions_raw (
            id, created_at, created_by, format, library_content_version_id, name, preview_image_id,
            preview_image_url, thumbnail_image_id, thumbnail_image_url, size, version,
            version_creation_method, workspace_content_id, modified_at
        ) values (
            %(id)s, %(createdAt)s, %(createdBy)s, %(format)s, %(libraryContentVersionId)s, %(name)s, %(previewImageId)s,
            %(previewImageUrl)s, %(thumbnailImageId)s, %(thumbnailImageUrl)s, %(size)s, %(version)s,
            %(versionCreationMethod)s, %(workspaceContentId)s, %(modifiedAt)s
...
def main_job(repeat_interval_hours: int = None):
    ...
    while modified_at_start_time < datetime.datetime.now(tz=datetime.timezone.utc):
        ...
        log.info(f'Looking for workspace content versions modified between {modified_at_start_time_s} and {modified_at_end_time_s}')
        params = {
